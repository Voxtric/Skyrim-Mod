Scriptname DLC1RedwaterDenSetStageAliasScript extends ReferenceAlias  
...
auto STATE waitingForPlayer
	...
	Event OnHit(ObjectReference akAggressor, Form akWeapon, Projectile akProjectile, bool abPowerAttack, bool abSneakAttack, bool abBashAttack, bool abHitBlocked)
		...
		if (akWeapon as spell)
			debug.Trace(self + ": hit by spell " + akWeapon)
			if (akWeapon as Spell).isHostile()
				debug.Trace(self + ": spell " + akWeapon + " is Hostile")
				testForTrigger(akAggressor, 2)
			else
				debug.Trace(self + ": spell " + akWeapon + " is NOT Hostile")
			endif
		else
			testForTrigger(akAggressor, 2)
		endif
