...
Scriptname BYOH_QF_BYOHHouseFalkreath_0100305E Extends Quest Hidden
...
kmyQuest.BYOHHouseBuilding.SetStage(100)
...
; create letter
; if player is already friends with Jarl, send friend letter
...
debug.trace(self + "stage 12 - sending intro note")
; create letter
; send intro letter
...
; send courier
WICourier.addItemToContainer(newLetter)
; remove Jarl from favor 110 faction
actor jarl = Alias_Jarl.GetActorRef()
jarl.RemoveFromFaction(Favor110QuestGiverFaction)
; if Jarl is currently running favor 110, finish it
if jarl == Favor110Questgiver.GetActorRef()
	Favor110.SetStage(200)
endif
; if Jarl is currently not a friend, make him one
if jarl.GetRelationshipRank(Game.GetPlayer()) < 1
	jarl.SetRelationshipRank(Game.GetPlayer(), 1)
endif
;END CODE
EndFunction
;END FRAGMENT
...
{Siddgeir's initial favor quest faction - pull this when sending intro letter}

Quest Property Favor110  Auto  
{Siddgeir's initial favor quest - complete if running when intro letter is sent}
